...
from get_data import get_label, get_workload, print_xml_stream, get_pce_health
...
class Illumio:
    ...
    def __init__(self):
        ...
        self.private_ip = self.config.get("private_ip", "")
        ...
        if self.api_key and self.api_secret:
            ...
            if self.enable_data_collection and self.enable_data_collection == 1:
                self.enable_scripted_input()
    ...
    def enable_scripted_input(self):
        try:
            r = splunk.rest.simpleRequest(
                "/servicesNS/nobody/" + app_name + "/data/inputs/script/%24SPLUNK_HOME%252Fetc%252Fapps%252F" + app_name + "%252Fbin%252FreadServicesFile.py/enable",
                self.session_key, method='POST')
        except:
            logger.exception("Unable to enable scripted input")
            raise Exception
    ...
    def update_mod_inputs(self, config):
        path = urllib.quote(self.mod_input_name, safe='')
        try:
            ...
            if len(result_storage_password["entry"]) > 0:
                for ele in result_storage_password["entry"]:
                    if ele["name"] == self.mod_input_name:
                        ...
                        try:
                            r = splunk.rest.simpleRequest(url, self.session_key,
                                                          postargs={"api_secret": config["api_secret"],
                                                                    "api_key_id": config["api_key_id"],
                                                                    "cnt_port_scan": ele["content"]["cnt_port_scan"],
                                                                    "pce_url": ele["content"]["pce_url"],
                                                                    "port_number": int(ele["content"]["port_number"]),
                                                                    "interval": interval,
                                                                    "time_interval_port": int(
                                                                        ele["content"]["time_interval_port"].split(".")[
                                                                            0])},
    ...
    def rest_help(self):
        return [self.pce_url, self.api_key, self.api_secret, self.cert_path,self.org_id]
    ...
    def print_ps_details(self):
        ...
        if allowed_ip:
            res = {"pce_url": pce_url, "port_scan": port_scan, "interval": interval,"illumio_type": "illumio:pce:ps_details","allowed_ip":allowed_ip}
        else:    
            res = {"pce_url": pce_url, "port_scan": port_scan, "interval": interval,"illumio_type": "illumio:pce:ps_details"}
        res = json.dumps(res)
    ...
    <endpoint>
        <args>
            ...
            <arg name="qurantine_label">
                ...
                <required_on_create>false</required_on_create>
                <validation>
                    validate(match(qurantine_label, '(\S+,\S+,\S+)'), "Enter three labels of type app, loc and env")
                </validation>
            </arg>
            <arg name="allowed_ip">
                <title>Comma Separated list of hosts, which will be ignored in Port scans</title>
                <description>Port scans from these hosts are ignored</description>
                <required_on_edit>false</required_on_edit>
                <required_on_create>false</required_on_create>
            </arg>
            <arg name="private_ip">
                <title>Privat IP address of Illumio Nodes</title>
                <description>Comma Separated IP address of all the nodes managed by this PCE instance.</description>
                <required_on_edit>false</required_on_edit>
                <required_on_create>false</required_on_create>
            </arg>
            <arg name="org_id">
                <title>Oragnisation ID</title>
                <description>This Org-ID will be used for making REST API calls to PCE.</description>
                <required_on_edit>false</required_on_edit>
                <required_on_create>false</required_on_create>
                <validation>
                    validate(is_pos_int(cnt_port_scan), "Count for port scan: must be non negative integer.")
                </validation>
            </arg>
        </args>
    </endpoint>
...
def get_validation_data():
    """
    This method fetches configuration parameters as passed by Splunk as XML when executing this Modular Input.
    :return:
    """
    val_data = {}
...
def validate_arguments():
    """
    This method validates different input arguments of Modular Input page. If the value is invalid, appropriate message is
    displayed on screen
    ...
    private_ip = val_data.get("private_ip", "")
    ...
    if api_key_id and api_secret:
        ...
        if not bool(re.search('^(https://)\S+', pce_url)):
            print_error("PCE URL: PCE URL must begin with 'https://'", session_key)

        port_check_regex = '^([1-9][0-9]{0,3}|[1-5][0-9]{4}|6[0-4][0-9]{3}|65[0-4][0-9]{2}|655[0-3][0-9]|6553[0-5])$'
        ...
        auth = "Basic " + base64.encodestring('%s:%s' % (api_key_id, api_secret)).replace('\n', '')
        ...
        try:
            ...
            print_error("Path for the custom certificate: Invalid certificate file", session_key)
        except:
            print_error("Authentication Failed: Invalid PEC URL or API key id or API Secret", session_key)
            ...
            try:
                ...
                if len(labels) ==3:
                    ...
                    if app_label and env_label and loc_label:
                        writeconf("TA-Illumio",session_key,"illumio",pce_url,{"app":app_label,"env":env_label,"loc":loc_label})
                else:
        ...
        if private_ip:
            try:
                ip_list = private_ip.split(",")
                for ip in ip_list:
                    if not is_ip(ip):
                        print_error("Invalid value for Private IP", session_key)

                search_query = "| makeresults  | eval host=\""+private_ip+"\", fqdn=\""+re.match("https?:\/\/([a-zA-z0-9.]*):?",pce_url).group(1)+"\" | makemv delim=\",\" host " \
                "| mvexpand host | dedup fqdn host | eval _key = host +\"_\"+ fqdn | table _key host hostname fqdn | outputlookup illumio_host_details_lookup append=T"

                logger.info("Search_Query_Private_IP:::"+search_query)
                splunk_search.searchAll(search_query, earliest_time = "-60m", latest_time = "+5m", sessionKey = session_key,
                                          namespace = "IllumioAppforSplunk")

            except Exception:
                logger.exception("Error in Validating Privat IP")


def run_script():
    illumio = Illumio()
    '''
        To ensure data collection is enabled only if choosen.
    '''
    if illumio.enable_data_collection ==1:
        ...
        get_pce_health(arg)
        logger.info("Flushing data from std out.")
        sys.stdout.flush()
        ...
        t1.join()
        t2.join()
        logger.info("Completed execution of threads.")
